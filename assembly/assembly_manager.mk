...
$(S_INPUT_DONE):
	...
	$(MAKE) m=par par \
		...
		PAR_DISK_GB=$(ASSEMBLY_INPUT_DISK_GB) \
		PAR_DISK_TYPE=$(ASSEMBLY_INPUT_DISK_TYPE) \
		PAR_PREEMTIBLE=$(ASSEMBLY_INPUT_PREEMTIBLE) \
...
S_ASSEMBLY_TABLE_DONE?=$(ASSEMBLY_MULTI_DIR)/.done_table
$(S_ASSEMBLY_TABLE_DONE):
	$(call _start,$(ASSEMBLY_MULTI_DIR))
	$(_R) R/assembly_table.r make.table \
		ifn=$(ASSEMBLY_LIBS_INPUT_TABLE) \
		ofn=$(ASSEMBLY_TABLE)
	$(_end_touch)
s_assembly_table: $(S_ASSEMBLY_TABLE_DONE)

ASSEMBLY_MULTI_DONE?=$(ASSEMBLY_MULTI_DIR)/.done_run
$(ASSEMBLY_MULTI_DONE): $(S_ASSEMBLY_TABLE_DONE)
	$(_start)
