...
uniref_diamond_index:
	$(MAKE) genes_diamond_index \
...
$(UNIREF_BLAST_DONE):
	...
	@$(MAKE) genes_diamond_blast \
		BLAST_DIR=$(UNIREF_DIR) \
		BLAST_QUERY=$(GENE_FASTA_AA) \
...
$(UNIREF_UNIQ_DONE): $(UNIREF_BLAST_DONE)
	$(_start)
	$(call _time,$(UNIREF_DIR),uniq) perl $(_md)/pl/blast_to_uniq.pl \
		$(UNIREF_RAW_OFN) \
		$(UNIREF_OFN_UNIQUE)
	$(_end_touch)
...
$(UNIREF_GENE_TAX_DONE): $(UNIREF_UNIQ_DONE)
	$(_start)
	$(call _time,$(UNIREF_DIR),lookup_uniref) \
		perl $(_md)/pl/lookup_uniref.pl \
			$(UNIREF_TABLE) \
			$(UNIREF_OFN_UNIQUE) \
...
genes_uniref_init: $(UNIREF_GENE_TABLE) $(UNIREF_TAX_LOOKUP_DONE)
	$(_start)
	mkdir -p $(UNIREF_DIAMOND_DB_DIR)
	@$(MAKE) genes_diamond_index \
		BLAST_DIR=$(UNIREF_DIR) \
