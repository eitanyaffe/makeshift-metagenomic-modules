...
$(S_EXTRACT_DONE): $(S_SETUP_DIRS_DONE)
	...
	$(MAKE) m=par par \
		...
		PAR_EMAIL=F \
		PAR_MACHINE=$(LIBS_BASIC_MACHINE) \
...
$(PP_COUNT_TRIMMOMATIC): $(S_TRIMMO_DONE)
	$(_start)
	perl $(_md)/pl/count_fastq.pl $(TRIMMOMATIC_OUTDIR) 'paired_' '.fastq' no_adapters $@
	$(_end)
trimmo_stats: $(PP_COUNT_TRIMMOMATIC)

LIB_DUPS_MACHINE?=n1-highmem-8
...
############################################################################
S_RAW_DONE?=$(LIB_INFO_DIR)/.done_raw
$(S_RAW_DONE): $(S_SETUP_DIRS_DONE)
	$(_start)
	$(MAKE) m=par par \
		PAR_MODULE=libs \
		PAR_NAME=lib_unzip \
